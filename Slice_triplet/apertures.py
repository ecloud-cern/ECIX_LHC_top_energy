...
def plot_in_triplet(s, beams=beams):
    ...
    bs007 = plot_rectcircle(halfwidth=0.0347, halfheight=0.0298, radius=0.0347)
    bs014 = plot_rectcircle(halfwidth=0.0298, halfheight=0.0347, radius=0.0347)
    if abs(s - ip5_s) < abs(ip1_s):
        if abs(s - ip5_s) < 31.40:
            beamscreen = bs010 
        elif abs(s - ip5_s) < 54.882: 
            beamscreen = bs012 
        else:
            beamscreen = bs014 
    else:
...
for s in np.linspace(ip5_s + 23, ip5_s + 58, 10):
