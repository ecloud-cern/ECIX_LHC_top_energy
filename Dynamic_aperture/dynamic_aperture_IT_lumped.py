...
num_r = 200#int(np.sqrt(num_particles))
num_theta = 90#int(np.sqrt(num_particles))
...
ecloud_info = {}
for it in ['itr1', 'itl1', 'itr5', 'itl5']:
    for key in eclouds.keys():
        if it in key:
            index = key.split('.')[3]
            name = key
            ecloud_info[it] = {name: {'length': eclouds[key]['length'],
                                           's': eclouds[key]['s']
                                          }}
...
filenames = {'itr1' : 'itr1.h5', 'itl1' : 'itl1.h5',
             'itr5' : 'itr5.h5', 'itl5' : 'itl5.h5',
             }
